...
def receive_selection(selection: SelectionModel):
    ...
    response_message1 = f"You selected option number: {selection.bedroom_count} and {selection.facade_count} and {selection.has_assigned_city_10} and {selection.net_habitable_surface} and {selection.land_surface}. Ordinal selected are: {selection.kitchen_type_ord_enc} and {selection.building_condition_ord_enc} and {selection.epc_ord_enc}"
    ...
    feature_names = ['bedroom_count', 'net_habitable_surface', 'facade_count', 'land_surface', 'has_assigned_city_10', 
                       'kitchen_type_ord_enc', 'building_condition_ord_enc', 'epc_ord_enc', 'locality_code']

    #new_house = np.array([3,200,4,500,1,3,3,3]).reshape(1,-1)
    ...
    # Convert the 'locality_code' column to string type
    df_new_house['locality_code'] = df_new_house['locality_code'].astype(str)
    df_new_house = map_province(df_new_house)
    df_new_house = df_new_house.drop(columns='locality_code')
    ...
    response_message = f"Price prediction in euro: {round(predictions[0])}"    # Return the Result
